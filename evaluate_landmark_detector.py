...
def main():
    ...
    parser.add_argument('--model_name', type=str, default="sdm_landmark_regressor_lfpw_r5_p32_s10", help='Name of the model in the saved models\' directory.')
    args = parser.parse_args()

    model_output_dir = os.path.join(args.output_dir, args.model_name)
    os.makedirs(model_output_dir,exist_ok=True)
    model_load_path= os.path.join(args.model_dir, f"{args.model_name}.model")
    ...
    for index_id, (image, real_landmark_coordinates) in tqdm(enumerate(zip(images,df_landmarks_test.values)),desc=f'Predicting the landmarks',total=len(images)):
        ...
        output_image = cv2.putText(output_image, f'RMSE: {round(rmse,3)}', (width//2-60, height+29), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
